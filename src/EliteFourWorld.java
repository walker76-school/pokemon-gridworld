...
/**
 * World for battling the Elite Four
 * @author Andrew
 */
public class EliteFourWorld
{
	private static Player player;
	
	/**
	 * creates an EliteFourWorld where a player can pick their name as well as six Pokemon for their party. After picking the
	 * Pokemon they will pick the moves for each Pokemon. Afterwards a battle with Elite Four Sidney will start.
	 */
    public EliteFourWorld()
    {
    	...
    	for(int i  = 0; i < 6; i++)
    	{
    		pickPokemon(player);
    		for(int j = 0; j < 4; j++)
    			pickMoves(player.getParty()[i]);
    	}  
			Sidney.doBattle(player);
    }	
    
    /**
     * screen to pick the Pokemon to battle with
     * @param player the player who's party will be selected
     */
        ...
        for (int r = 1; r < 100; r++) {
            for (int c = 0; c < 6; c++) {
            	if(index < pokemon.length)
            	{
            		...
            		for(Pokemon poke: player.getParty())
            			if(poke != null && poke.getName().equals(p.getName()))
    ...
    /**
     * pick moves for a specific Pokemon
     * @param poke the Pokemon who's moves will be selected
     */
            ...
            for (int c = 0; c < 4; c++) {
            	if(index < moves.size())
            	{
            		...
            		if(m.getType().equals(Type.NORMAL))
            			button.setStyle("-fx-background-color: #A8A878; -fx-text-fill: white;");
