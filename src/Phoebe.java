...
/**
 * Elite Four Phoebe who holds a pokemon party to battle
 * @author Andrew
 *
 */
public abstract class Phoebe extends Actor
{
	...
	/**
	 * Does battle with Elite Four Glacia
	 * @param p the player to do battle with
	 */
	public static void doBattle(Player p)
	{
		Pokemon[] temp = {new Pokemon("Phoebe's Dusclops"), new Pokemon("Phoebe's Banette"), new Pokemon("Phoebe's Dusclops"), new Pokemon("Phoebe's Banette"), new Pokemon("Phoebe's Sableye")};
		party = temp;	
		...
		AlertBox.display("Uh Oh!", "Elite Four Phoebe would like to battle!", "Let's Go!");
		...
		for(int i = 0; i < party.length; i++)
		{
			...
			{
				mp.stop();
				AlertBox.displayNoTitle("You have lost to Elite Four Phoebe. Click OK to end your game!", "OK");
				break;
			}
			...
			{				
				if(i < party.length - 1)
				{
					...
					newPokemonAlert.setHeaderText("Elite Four Phoebe is about to send out " + party[i+1].getName());
				...
				if(i == 5)
				{
					AlertBox.displayNoTitle("You beat Elite Four Phoebe!", "OK");
					mp.stop();
